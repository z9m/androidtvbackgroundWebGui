...
def get_random_media():
    ...
    if jf.get('url') and jf.get('api_key'):
        ...
        url = f"{clean_url}/Users/{jf['user_id']}/Items?Recursive=true&IncludeItemTypes=Movie,Series&ExcludeItemTypes=BoxSet&SortBy=Random&Limit=50&Fields=Overview,Genres,CommunityRating,ProductionYear,RunTimeTicks,ImageTags,Path,ProviderIds"
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Lato:wght@300;400;700&family=Merriweather:wght@300;400;700&family=Montserrat:wght@300;400;700&family=Open+Sans:wght@300;400;700&family=Oswald:wght@300;400;700&family=Playfair+Display:wght@400;700&family=Roboto:wght@300;400;700&display=swap" rel="stylesheet">
    <style>
        ...
        .gallery-item { position: relative; min-width: 300px; aspect-ratio: 16/9; overflow: hidden; border-radius: 4px; border: 1px solid #333; cursor: pointer; transition: transform 0.2s; flex-shrink: 0; }
...
<body>
    ...
    <div class="nav-tabs">
        ...
        <div class="tab-link" onclick="openTab(event, 'layouts-tab'); loadLayoutsList()">💾 Saved Layouts</div>
        <div class="tab-link" onclick="openTab(event, 'gallery-tab'); loadGallery()">🖼️ Gallery</div>
    ...
    <div id="editor-tab" class="tab-content active">
        <div class="sidebar">
            <div class="control-group">
                ...
                <button onclick="saveLayout()" style="background: #00796b; margin-top: 10px;">💾 Save Layout</button>
            </div>
            ...
            <div class="control-group">
                ...
                <div style="display:flex; gap:5px; align-items:center;">
                    ...
                    <button onclick="autoDetectBgColor(true)" style="margin-top:0; font-size: 11px;">✨ Auto-Detect</button>
                </div>
                ...
                <button onclick="mirrorBackground()" style="margin-top: 10px; background-color: #4a4a4a;">↔️ Mirror Background</button>
            </div>
            ...
            <div style="margin-top:auto">
                <button class="btn-export" onclick="saveImage()">💾 Export JPG (Download)</button>
                <button onclick="saveToGallery()" style="background: #e65100; margin-top: 10px;">📂 Save to Gallery</button>
            </div>
        </div>

        <div class="main-view">
            <button onclick="toggleGrid()" style="position:absolute; top:20px; left:20px; width:auto; z-index:100; background-color: #4a4a4a; padding: 8px 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.5);">📏 Toggle Grid</button>
    ...
    <script>
        ...
        let gridEnabled = false, movingObjects = [], snapLines = { v: [], h: [] }, guideLines = [];
        const gridSize = 50;

        function updateSelectionUI(e) {
            const activeObj = canvas.getActiveObject();
            const panel = document.getElementById('text-properties');
            if (activeObj && (activeObj.type === 'i-text' || activeObj.type === 'textbox' || (activeObj.type === 'group' && activeObj.dataTag === 'rating_star'))) {
        ...
        function updateSelectedFontSize() {
            ...
            if (activeObj) { 
                ...
                if (textObj) { 
                    textObj.set("fontSize", parseInt(document.getElementById('fontSizeInput').value)); 
        ...
        function updateSelectedFontFamily() {
            ...
            if (activeObj) { 
                let textObj = (activeObj.type === 'group') ? activeObj.getObjects().find(o => o.type === 'i-text') : activeObj; 
                if(textObj) { 
                    textObj.set("fontFamily", document.getElementById('fontFamilySelect').value); 
                    if(activeObj.type==='group') activeObj.addWithUpdate(); 
                    canvas.renderAll(); 
                } 
            }
                ...
                [...canvas.getObjects()].forEach(obj => {
                    if (obj.dataTag) {
                        ...
                        switch(obj.dataTag) {
                            ...
                            case 'rating_val': let rv = mediaData.rating || mediaData.CommunityRating; val = (rv && rv !== 'N/A') ? `${rv}` : ''; break;
                            case 'rating_star': 
        ...
        function autoDetectBgColor(forceRecalc) {
            if (!mainBg) return;
        ...
        function addMetadataTag(type, placeholder) {
            ...
            const props = { left: 100 + (count * 30), top: 100 + (count * step), fontFamily: 'Segoe UI', fontSize: type === 'title' ? titleSize : baseSize, fill: 'white', shadow: '2px 2px 10px rgba(0,0,0,0.8)', dataTag: type };
        ...
        function updateVerticalLayout() {
            const padding = 20; // Vertical padding between elements
            const anchor = canvas.getObjects().find(o => o.dataTag === 'title');
            if (!anchor) return;

            let lastBottom = anchor.top + anchor.getScaledHeight();

            const elements = canvas.getObjects().filter(o => o.dataTag && o.dataTag !== 'title' && o.dataTag !== 'guide' && o.visible);
            elements.sort((a, b) => a.top - b.top);

            elements.forEach(el => {
                el.set('top', lastBottom + padding);
                el.setCoords();
                lastBottom = el.top + el.getScaledHeight();
            });
            canvas.requestRenderAll();
        }
        ...
        function drawGrid() {
            ...
            const opts = { stroke: '#555', strokeDashArray: [5, 5], selectable: false, evented: false, isGridLine: true };
        ...
        function updateFades(skipRender = false) {
            ...
            Object.values(fades).forEach(obj => { if(obj) canvas.remove(obj); });
            fades = { left: null, right: null, top: null, bottom: null, corner: null };
        ...
        function addCornerFade(pos) {
            ...
            fades.corner = new fabric.Rect({ left: rectLeft, top: rectTop, width: r, height: h, fill: grad, selectable: false, evented: false });
        ...
        function addVignette() {
            ...
            fades.corner = new fabric.Rect({ left: mainBg.left, top: mainBg.top, width: w, height: h, fill: grad, selectable: false, evented: false });
            canvas.add(fades.corner);
            fades.corner.moveTo(canvas.getObjects().indexOf(mainBg) + 1);
        }
            ...
            try {
                ...
                for (const [folder, images] of Object.entries(loadedGalleryData)) {
                    ...
                    images.forEach((img, index) => {
                        ...
                        html += `
                            <div class="gallery-item" onclick="openLightbox('${folder}', ${index})">
        ...
        async function saveToGalleryInternal(layoutName) {
            const dataURL = canvas.toDataURL({ format: 'jpeg', quality: 0.95 });
            const payload = { image: dataURL, layout_name: layoutName };
            if (lastFetchedData) {
        ...
        async function saveLayout() {
            ...
            const layout = canvas.toJSON(['dataTag', 'fullMediaText', 'selectable', 'evented', 'lockScalingY', 'splitByGrapheme', 'fixedHeight', 'editable']);
        ...
        async function loadLayoutsList() {
            ...
            layouts.forEach(l => {
                ...
                if (loadedGalleryData[key]) {
                    loadedGalleryData[key].slice(0, 5).forEach((img, index) => {
